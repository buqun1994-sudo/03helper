...
历史版本字段不属于当前 v4 主链；回滚通过新 `catalogRevision` 指向仍可用的旧目录 / 文件集合完成。客户端按 `environment + channel` 持久化最高修订号，旧签名快照不能覆盖新状态。
...
3. 全部 ZIP 维护在同一个受密码保护的蓝奏入口中；推荐使用带版本的唯一 ZIP 文件名，避免覆盖仍可能被复用的旧文件。目录可包含人工维护的其它文件；客户端只处理签名 `apps[]` 声明的 ZIP，任一声明 ZIP 都允许在目录阶段暂时缺失，因为公共 `Download` 可能已有可复用 APK。
...
5. 只有目标环境的下载、解压、APK 包身份 / 证书校验和 ADB 安装验证全部通过，才允许把签名配置切到公开状态。当前 production revision 5 四组件候选已完成本地原始字节验签和客户端 Release 单测，但使用占位入口且 Fossify Logo 公网对象尚未上传，因此保持 `local-only` / `publishable=false`，不代表 Cloud production 配置已发布。
...
3. M2：由 Cloud 发布方上传并核验 ZIP / 文件完整性和 Logo 对象，继续维护真实蓝奏入口并再次生成更高 revision 候选；完成 production 小范围下载、解压、APK 身份、ADB 安装和反回滚验证后，才可按“上线”门禁发布公开配置。若未来增加 R2 / GitHub 备用，再另行验证切源。
