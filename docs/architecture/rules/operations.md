...
3. 触发条件：准备发布或切换下载源。动作：先生成一次单组件 ZIP 并镜像同一字节文件，版本清单分别记录 ZIP / APK 哈希、大小、签名摘要、兼容范围和回滚指针，再做小范围验证。验证：隐藏 WebView 下载回调、ZIP 解压、APK 校验、回滚和缓存命中；稳定且不超过 5 分钟的最小检查可由 AI 执行，复杂真实链路交付人工用例。边界：不把个人存储、密码文件夹、合并 ZIP 或未审查镜像当自动正式源。
