...
3. 触发条件：准备发布或检查组件更新。动作：为三个组件各生成只含一个 APK 的 ZIP，放入同一个受密码保护的根文件夹；服务端签发配置中的文件名、包名和证书白名单，客户端动态读取 APK 版本 / 大小 / 哈希后再生成清单。验证：隐藏 WebView 密码验证与目录枚举、ZIP 解压、APK 校验、目录替换后的版本识别和缓存清理；稳定且不超过 5 分钟的最小检查可由 AI 执行，复杂真实链路交付人工用例。边界：不把未签名配置、根目录外文件、合并 ZIP、任意直链或未审查镜像当自动正式源。
