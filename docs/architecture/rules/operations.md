...
1. 触发条件：需要本机构建、ADB 或设备验证。动作：先读取 `.codex/local-context.properties`，再执行环境检查；每次连接车机先用首选 ADB 快速枚举，未发现已验证车机时立即做一次当前局域网的有界短超时 TCP 候选枚举，只有 IP 的候选默认补 `:5555`。历史 serial 只作提示；候选连接后以显式 serial 核对 `device`、`S56_HQX` 和 SDK 28。验证：环境快检和本轮确认的定向设备列表。边界：只有现状枚举与候选连接均失败才报告离线；不猜测个人路径，不高频或无界扫描，不枚举全部在线设备执行破坏性任务，也不并行启动冲突 ADB Server。
