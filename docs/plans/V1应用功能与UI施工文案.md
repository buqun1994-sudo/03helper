...
| 找到设备 | `选择要连接的车机` | `选择后将自动检查连接状态。` | 点击设备行 |
...
2. 已连接成功后自动进入“选择应用”页；取消或失败时保留可重试入口。

### 5.2 选择应用

1. 页面标题为 `选择要安装的应用`，说明为 `03歌词和03桌面会一并安装。`。
...
1. 接入局域网发现、显式设备选择、ADB 传输、非流式推送、受控授权和可用性验证。
2. 实现取消、断线和检查点恢复；所有用户可见进度从 `InstallationSession` 的结构化结果驱动。
3. OTG 不与 LAN 主链同时施工，只有独立能力证据通过后再评审。
