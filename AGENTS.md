...
1. 文档、规则、Skill 或脚本变更至少执行：
   ...
   - `git diff --check`
2. 代码或工程行为变更默认按“语法 / 类型或编译 / 直接相关自动化测试 / 运行级 smoke / 最小人工确认”的顺序闭环；编译通过不等于安装流程可用。若用户在本轮明确指定“最简自动化 + 用户人工主测”，则以语法、编译和直接相关自动化测试为 AI 必做项，不擅自启动设备、浏览器或真实网络。
3. 涉及连接、下载、安装、授权、启动、导航或输入时，默认必须尝试真实运行级 smoke；但用户明确排除 AI smoke 并承担人工主测时，本轮将运行级验证标记为“待用户主测”，交付精确动作、预期结果、停止条件和客观阻断，不把 fixture 或模拟结果写成真实通过。
