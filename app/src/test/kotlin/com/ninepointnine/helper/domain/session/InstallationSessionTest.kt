...
class InstallationSessionTest {
    ...
    @Test
    fun `maintenance inventory keeps a separate validated third party list`() {
        val session = maintenanceSession()
        session.dispatch(InstallationSessionCommand.MaintenanceAction(MaintenanceActionId.MANAGE_APPS))

        val thirdParty = ThirdPartyApplicationStatus(
            packageName = "com.example.music",
            versionLabel = "1.2.3",
            installTimeEpochMillis = 200L,
            filePath = "/data/app/com.example.music-a/base.apk",
        )
        session.dispatchEvent(
            InstallationSessionEvent.MaintenanceApplicationsResolved(
                applications = emptyList(),
                thirdPartyApplications = listOf(thirdParty),
            ),
        )

        assertEquals(MaintenanceInventoryState.READY, session.currentSnapshot().maintenance.managedApplicationsState)
        assertEquals(listOf(thirdParty), session.currentSnapshot().maintenance.thirdPartyApplications)
        assertTrue(session.currentSnapshot().maintenance.managedApplications.isEmpty())
    }

    @Test
    fun `invalid third party path fails the maintenance inventory closed`() {
        val session = maintenanceSession()
        session.dispatch(InstallationSessionCommand.MaintenanceAction(MaintenanceActionId.MANAGE_APPS))
        session.dispatchEvent(
            InstallationSessionEvent.MaintenanceApplicationsResolved(
                ...
                thirdPartyApplications = listOf(
                    ThirdPartyApplicationStatus(
                        ...
                        filePath = "/system/app/com.example.system/base.apk",
                    ),
                ),
            ),
        )
    ...
    @Test
    fun `known third party row can enter every fixed maintenance action`() {
        MaintenanceApplicationActionId.values().forEach { actionId ->
            val session = maintenanceSession()
            session.dispatch(InstallationSessionCommand.MaintenanceAction(MaintenanceActionId.MANAGE_APPS))
            session.dispatchEvent(
                InstallationSessionEvent.MaintenanceApplicationsResolved(
    ...
    @Test
    fun `forged or controlled duplicate third party row is rejected`() {
        val forged = maintenanceSession()
        forged.dispatch(InstallationSessionCommand.MaintenanceAction(MaintenanceActionId.MANAGE_APPS))
        ...
        forged.dispatch(
            InstallationSessionCommand.MaintenanceApplicationAction(
                componentId = "third-party:com.example.missing",
                actionId = MaintenanceApplicationActionId.START,
            ),
        )
        assertEquals(
            "maintenance_component_unavailable",
            forged.currentSnapshot().maintenance.applicationAction?.reasonCode,
        )

        val duplicate = maintenanceSession()
        duplicate.dispatch(InstallationSessionCommand.MaintenanceAction(MaintenanceActionId.MANAGE_APPS))
        duplicate.dispatchEvent(
            InstallationSessionEvent.MaintenanceApplicationsResolved(
                ...
                thirdPartyApplications = listOf(
                    ThirdPartyApplicationStatus(
                        packageName = "com.tcrrry.desktop",
                        filePath = "/data/app/com.tcrrry.desktop-a/base.apk",
                    ),
                ),
            ),
        )
        ...
        duplicate.dispatch(
            InstallationSessionCommand.MaintenanceApplicationAction(
                componentId = "third-party:com.tcrrry.desktop",
                actionId = MaintenanceApplicationActionId.UNINSTALL,
            ),
        )
        assertEquals(
            "maintenance_component_unavailable",
            duplicate.currentSnapshot().maintenance.applicationAction?.reasonCode,
        )
    }
