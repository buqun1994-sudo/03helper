    ...
    fun toDomainOrNull(): AuthorizationSetupDeclaration? {
        return try {
            val parsedAppOps = appOps.map { value ->
                com.ninepointnine.helper.domain.device.ManagedAppOp.entries.first {
                    it.name == value || it.wireName == value
                }
            }
            val parsedRuntimePermissions = runtimePermissions.map { value ->
                com.ninepointnine.helper.domain.device.ManagedRuntimePermission.entries.first {
                    it.name == value || it.wireName == value
                }
            }
            val parsedSecureSettings = secureSettings.map { value ->
                com.ninepointnine.helper.domain.device.ManagedSecureFlag.entries.first {
                    it.name == value || it.wireName == value
                }
            }
            val parsedSecureComponents = secureComponents.map {
                AuthorizationSetupDeclaration.SecureComponent(
                    setting = com.ninepointnine.helper.domain.device.ManagedSecureComponentList.entries.first { setting ->
